...
def start(bot, update):
    ...
    update.message.reply_text(
        'Hola ' + name + ' soy TramBot escribe /help para obtener ayuda.')
    ...
    if(len(args) == 4):
        ...
        if(args[0] == args[1]):
            ...
            if(len(args[2]) == 5 and len(args[3]) == 5 and args[2][2] == ':'and args[3][2] == ':'and args[2][0].isdigit() and args[2][1].isdigit() and args[2][3].isdigit() and args[2][4].isdigit() and args[3][0].isdigit() and args[3][1].isdigit()and args[3][3].isdigit() and args[3][4].isdigit()):
                ...
                # Valida si ha encontrado la codificacion de la parada de
                # origen y destino
                if((numid in "Nothing") or (numid2 in "Nothing")):
                    update.message.reply_text(
                        "Error en las paradas.\nEs posible que haya errores en los nombres de las paradas")
                else:
                    h1 = args[2][0] * 1000 + args[2][1] * \
                        100 + args[2][3] * 10 + args[2][4]
                    h2 = args[3][0] * 1000 + args[3][1] * \
                        100 + args[3][3] * 10 + args[3][4]
                    if(h1 < h2):
                        ...
                        # Construyo y realizo la peticion con los datos
                        # correspondientes y los intervalos
                        payload = {
                            'origen': numid,
                            'aceptar': '0',
                            'key': '0',
                        ...
                        numero0 = '-1'
                        # Mando el horario de la forma: en una linea la hora y en la siguiente todas las
                        # veces que sale en esa hora, y asi con todas las
                        # horas.
                        ...
                        for hora in cadatd:
                            ...
                            if(hora[0] == numero0 and hora[1] == numero1):
                                ...
                                total = (total + hora + ' ')
                            # detecta cuando tiene que pasar a la siguiente
                            # hora
                            else:
                                # Detectar transbordo
                                if(numero0 * 10 + numero1 > hora[0] * 10 + hora[1]):
                        ...
                        if(total == ''):
                            update.message.reply_text(
                                'Lo siento, no sale ningun tram en el intervalo horario indicado.')
                        else:
                            update.message.reply_text('Salidas desde ' + str.lower(args[0]) + ' a ' + str.lower(
                                args[1]) + ' de ' + hinicio + ' a ' + hfinal + ':' + total)
                    else:
                        update.message.reply_text(
                            'Error.\nLa hora de inicio debe ser menor que la hora de fin.')
            else:
                update.message.reply_text(
                    'El intervalo de horas es incorrecto, debe cumplir este formato: XX:XX')
    else:
        update.message.reply_text(
...
def main():
    ...
    dp.add_handler(telegram.ext.CommandHandler(
        "times", timestoday, pass_args=True))
    dp.add_handler(telegram.ext.CommandHandler(
        "timesto", timesintervalo, pass_args=True))
