    ...
    if (len(args) == 2):
        ...
        if(args[0]==args[1]):
            ...
            if((numid in "Nothing") or (numid2 in "Nothing")):
                ...
                numero1='0'
                numero0='0'
                #Mando el horario de la froma, en una linea la hora y en la siguiente todas las
                ...
                update.message.reply_text('Salidas desde **'+args[0]+'** a **'+args[1]+ '** :\n'+total)
...
#
#  [+] Implementar: obtener la salida y destino (en plan): "times SantaPolla AlacantPuteros"
#     obtener ese destino y salida, obtener numeros de la estacion y añadirlos a la queryhttp
#
def timesintervalo(bot,update,args):
    ...
    if(len(args) == 4):
        ...
        cadatd = re.findall("(\d+:\d+)", args[2])
        update.message.reply_text('Pillado '+ args[0])
        ...
        if(args[0]==args[1]):
			...
			'	origen': '124',
                ...
                #Scrapeo cada valor de la tabla de horarios
                cadatd = re.findall("<td>(\d+:\d+)</td>", r.text)
                print('Args: %s' % args[0:4])
            else:
                update.message.reply_text('El intervalo de horas es incorrecto, deben ser valores numéricos!')
    else:
